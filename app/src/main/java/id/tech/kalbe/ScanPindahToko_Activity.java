...
public class ScanPindahToko_Activity extends FragmentActivity implements
	...
	public void handleDecode(Result rawResult, Bitmap barcode) {
		...
		sh_pf.edit().putString(Parameter_Collections.SH_KODE_TOKO_SEMENTARA, code).commit();
...
//			new Async_SubmitAbsen().execute();
			if(extra_dari_inputstok){
				Intent load = new Intent(getApplicationContext(), DialogChooserInputStok.class);
				startActivity(load);
				finish();
			}else{
				Intent load = new Intent(getApplicationContext(), DialogChooserInputPennjualan.class);
				startActivity(load);
				finish();
			}
